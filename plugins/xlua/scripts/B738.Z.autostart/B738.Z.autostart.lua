...
simDR_gpu_status						= find_dataref("sim/cockpit/electrical/gpu_on")
...
B738DR_init_systems_CD					= find_dataref("laminar/B738/init_CD/systems")
B738DR_init_annun_CD					= find_dataref("laminar/B738/init_CD/annun")
B738DR_init_chrono_CD					= find_dataref("laminar/B738/init_CD/chrono")
B738DR_init_comms_CD					= find_dataref("laminar/B738/init_CD/comms")
B738DR_init_fire_CD						= find_dataref("laminar/B738/init_CD/fire")
B738DR_init_glare_CD					= find_dataref("laminar/B738/init_CD/glare")
B738DR_init_lighting_CD 				= find_dataref("laminar/B738/init_CD/lighting")
B738DR_init_gear_CD						= find_dataref("laminar/B738/init_CD/gear")
...
B738CMD_apuL_switch_dn					= find_command("laminar/B738/switch/apuL_dn")
B738CMD_apuR_switch_dn					= find_command("laminar/B738/switch/apuR_dn")

B738CMD_apuL_switch_up					= find_command("laminar/B738/switch/apuL_up")
B738CMD_apuR_switch_up					= find_command("laminar/B738/switch/apuR_up")

B738CMD_gen1_switch_dn					= find_command("laminar/B738/switch/gen1_dn")
B738CMD_gen2_switch_dn					= find_command("laminar/B738/switch/gen2_dn")

B738CMD_gpu_pos_dn						= find_command("laminar/B738/toggle_switch/gpu_pos_dn")

-- AI QUICKSTART

B738CMD_ai_annun_quick_start			= find_command("laminar/B738/ai/annun_quick_start")
B738CMD_ai_chrono_quick_start			= find_command("laminar/B738/ai/chrono_quick_start")
B738CMD_ai_comms_quick_start			= find_command("laminar/B738/ai/comms_quick_start")
B738CMD_ai_fire_quick_start				= find_command("laminar/B738/ai/fire_quick_start")
B738CMD_ai_glare_quick_start			= find_command("laminar/B738/ai/glare_quick_start")
B738CMD_ai_lighting_quick_start			= find_command("laminar/B738/ai/lighting_quick_start")
B738CMD_ai_systems_quick_start			= find_command("laminar/B738/ai/systems_quick_start")
B738CMD_ai_gear_quick_start				= find_command("laminar/B738/ai/gear_quick_start")
...
function B738_autoboard_init()
	if is_timer_scheduled(B738_autoboard_phase_timeout) == true then
	    stop_timer(B738_autoboard_phase_timeout)                               
	end	
    autoboard.step = -1
    autoboard.phase = {}
    autoboard.sequence_timeout = false
...
function B738_auto_board()
    ...
    elseif autoboard.step == 12 then
        ...
        if autoboard.phase[autoboard.step] == 1 then
			...
			if B738DR_ground_power_avail_annun == 1 then									-- IF GPU AVAIL
				B738CMD_gpu_pos_dn:once()													-- TURN ON
	...
	----- AUTO-BOARD STEP 23: APU GENERATOR ON IF GPU NOT ON
    elseif autoboard.step == 23 then

        -- PHASE 1: SET THE SWITCH
        if autoboard.phase[autoboard.step] == 1 then
            B738_print_sequence_status(autoboard.step, autoboard.phase[autoboard.step], "SET APU GENERATORS TO ON IF GPU NOT AVAIL...")  -- PRINT THE START PHASE MESSAGE										
			if simDR_gpu_status == 1 then
			elseif simDR_gpu_status == 0 then
        ...
        if autoboard.phase[autoboard.step] == 3 then
            if autoboard.sequence_timeout == true then                                      -- PHASE FAILED
                B738_autoboard_step_failed(autoboard.step, autoboard.phase[autoboard.step])
            elseif simDR_apu_generator_switch == 1 or
            		simDR_apu_generator_switch == 0 and simDR_gpu_status == 1
            then                                                                            -- PHASE WAS SUCCESSFUL
                if is_timer_scheduled(B738_autoboard_phase_timeout) == true then
                    stop_timer(B738_autoboard_phase_timeout)                                -- KILL THE TIMER
                end
        ...
        -- PHASE 4: COMPLETE THE STEP
        if autoboard.phase[autoboard.step] == 4 then
            B738_autoboard_step_completed(autoboard.step, autoboard.phase[autoboard.step], "GPU AVAIL or APU GENERATORS ON")
        end
...
function B738_auto_start()
    ...
    elseif autostart.step == 1 then
        ...
        if autostart.phase[autostart.step] == 1 then
            B738_print_sequence_status(autostart.step, autostart.phase[autostart.step], "THROTTLE 1 TO IDLE CHECK..")  -- PRINT THE START PHASE MESSAGE
            simDR_throttle_pos_1 = 0														-- THROTTLE 1 IDLE
        ...
        if autostart.phase[autostart.step] == 4 then
            B738_autostart_step_completed(autostart.step, autostart.phase[autostart.step], "THROTTLE 1 IDLE")
        end


    ----- AUTO-START STEP 2: THROTTLE 2 IDLE CHECK
    elseif autostart.step == 2 then

        -- PHASE 1: SET THE SWITCH
        if autostart.phase[autostart.step] == 1 then
            B738_print_sequence_status(autostart.step, autostart.phase[autostart.step], "THROTTLE 2 TO IDLE CHECK..")  -- PRINT THE START PHASE MESSAGE
            simDR_throttle_pos_2 = 0														-- THROTTLE 2 IDLE
            autostart.phase[autostart.step] = 2                                             -- INCREMENT THE PHASE
        end
        ...
        -- PHASE 4: COMPLETE THE STEP
        if autostart.phase[autostart.step] == 4 then
            B738_autostart_step_completed(autostart.step, autostart.phase[autostart.step], "THROTTLE 2 IDLE")
        end



    ----- AUTO-START STEP 3: LEFT FUEL PUMPS ON
    ...
    ----- AUTO-START STEP 4: CENTER FUEL PUMPS ON
    ...
    ----- AUTO-START STEP 5: RIGHT FUEL PUMPS ON
    ...
    ----- AUTO-START STEP 6: ANTI-COLLISION LIGHTS ON
    ...
    ----- AUTO-START STEP 7: APU GEN L ON / GRD POWER OFF
    elseif autostart.step == 7 then

        -- PHASE 1: SET THE SWITCH
        if autostart.phase[autostart.step] == 1 then
            B738_print_sequence_status(autostart.step, autostart.phase[autostart.step], "APU GENERATOR LEFT TO ON...")  -- PRINT THE START PHASE MESSAGE
            B738CMD_apuL_switch_dn:once()													-- APU L GEN ON
            autostart.phase[autostart.step] = 2                                             -- INCREMENT THE PHASE
        end
        ...
        if autostart.phase[autostart.step] == 3 then
            if autostart.sequence_timeout == true then                                      -- PHASE FAILED
                B738_autostart_step_failed(autostart.step, autostart.phase[autostart.step])
            elseif B738DR_apu_genL_status == 1												-- APU GEN L STATUS
            	then                                                						-- PHASE WAS SUCCESSFUL
                if is_timer_scheduled(B738_autostart_phase_timeout) == true then
                    stop_timer(B738_autostart_phase_timeout)                                -- KILL THE TIMER
                end
        ...
        -- PHASE 4: COMPLETE THE STEP
        if autostart.phase[autostart.step] == 4 then
            B738_autostart_step_completed(autostart.step, autostart.phase[autostart.step], "APU GENERATOR LEFT ON, GROUND POWER OFF BUS")
        end

    ----- AUTO-START STEP 8: APU GEN R ON
    elseif autostart.step == 8 then

        -- PHASE 1: SET THE SWITCH
        if autostart.phase[autostart.step] == 1 then
            B738_print_sequence_status(autostart.step, autostart.phase[autostart.step], "APU GENERATOR RIGHT TO ON...")  -- PRINT THE START PHASE MESSAGE
            B738CMD_apuR_switch_dn:once()													-- APU R GEN ON
            autostart.phase[autostart.step] = 2                                             -- INCREMENT THE PHASE
        end
        ...
        if autostart.phase[autostart.step] == 3 then
            if autostart.sequence_timeout == true then                                      -- PHASE FAILED
                B738_autostart_step_failed(autostart.step, autostart.phase[autostart.step])
            elseif B738DR_apu_genR_status == 1												-- APU GEN R STATUS
            	then                                                						-- PHASE WAS SUCCESSFUL
                if is_timer_scheduled(B738_autostart_phase_timeout) == true then
                    stop_timer(B738_autostart_phase_timeout)                                -- KILL THE TIMER
                end
        ...
        -- PHASE 4: COMPLETE THE STEP
        if autostart.phase[autostart.step] == 4 then
            B738_autostart_step_completed(autostart.step, autostart.phase[autostart.step], "APU GENERATOR RIGHT ON")
        end
...
----------------------------------------------------
    ...
    ----- AUTO-START STEP 9: ENGAGE STARTER 2
    elseif autostart.step == 9 then
        ...
        if autostart.phase[autostart.step] == 1 then
            ...
            B738DR_starter_2_pos = -1														-- STARTER 2 ENGAGED
            autostart.phase[autostart.step] = 2                                             -- INCREMENT THE PHASE
        end
    ...
    ----- AUTO-START STEP 10: MONITOR ENGINE 2 N2
    ...
    ----- AUTO-START STEP 11: CONDITION LEVER 2 TO IDLE
    ...
    ----- AUTO-START STEP 12: MONITOR ENGINE 2 N2
    ...
    ----- AUTO-START STEP 13: DISENGAGE STARTER 12
...
----------------------------------------------------
    ...
    ----- AUTO-START STEP 14: ENGAGE STARTER 1
    elseif autostart.step == 14 then
        ...
        if autostart.phase[autostart.step] == 1 then
            ...
            B738DR_starter_1_pos = -1														-- STARTER 2 ENGAGED
            autostart.phase[autostart.step] = 2                                             -- INCREMENT THE PHASE
        end
    ...
    ----- AUTO-START STEP 15: MONITOR ENGINE 1 N2
    ...
    ----- AUTO-START STEP 16: CONDITION LEVER 1 TO IDLE
    ...
    ----- AUTO-START STEP 17: MONITOR ENGINE 1 N2
    ...
    ----- AUTO-START STEP 18: DISENGAGE STARTER 1
	...
	----- AUTO-START STEP 19: GENERATOR 2 ON
    elseif autostart.step == 19 then
        ...
        if autostart.phase[autostart.step] == 1 then
            B738_print_sequence_status(autostart.step, autostart.phase[autostart.step], "TURN GENERATOR 2 TO ON...")  -- PRINT THE START PHASE MESSAGE
           	B738CMD_gen2_switch_dn:once()													-- GEN 2 ON
            autostart.phase[autostart.step] = 2                                             -- INCREMENT THE PHASE
        end
    ...
    ----- AUTO-START STEP 20: GENERATOR 1 ON
    elseif autostart.step == 20 then
        ...
        if autostart.phase[autostart.step] == 1 then
            B738_print_sequence_status(autostart.step, autostart.phase[autostart.step], "TURN GENERATOR 1 TO ON...")  -- PRINT THE START PHASE MESSAGE
           		B738CMD_gen1_switch_dn:once()												-- GEN 1 ON
            autostart.phase[autostart.step] = 2                                             -- INCREMENT THE PHASE
        end
    ...
    ----- AUTO-START STEP 21: CAPTAIN PROBE HEAT ON
    ...
    ----- AUTO-START STEP 22: FIRST OFFICER PROBE HEAT ON
    ...
    ----- AUTO-START STEP 23: ENGINE 1 BLEED AIR ON
    ...
    ----- AUTO-START STEP 24: ENGINE 2 BLEED AIR ON
    ...
    ----- AUTO-START STEP 25: APU BLEED AIR OFF
    ...
    ----- AUTO-START STEP 26: APU GENERATOR L OFF
    elseif autostart.step == 26 then

        -- PHASE 1: SET THE SWITCH
        if autostart.phase[autostart.step] == 1 then
            B738_print_sequence_status(autostart.step, autostart.phase[autostart.step], "LEFT APU GENERATOR TO OFF...")  -- PRINT THE START PHASE MESSAGE
           	B738CMD_apuL_switch_up:once()													-- APU L GEN OFF
            autostart.phase[autostart.step] = 2                                             -- INCREMENT THE PHASE
        end
        ...
        if autostart.phase[autostart.step] == 3 then
            if autostart.sequence_timeout == true then                                      -- PHASE FAILED
                B738_autostart_step_failed(autostart.step, autostart.phase[autostart.step])
            elseif B738DR_apu_genL_status == 0
            	then                                                						-- PHASE WAS SUCCESSFUL
                if is_timer_scheduled(B738_autostart_phase_timeout) == true then
                    stop_timer(B738_autostart_phase_timeout)                                -- KILL THE TIMER
                end
        ...
        -- PHASE 4: COMPLETE THE STEP
        if autostart.phase[autostart.step] == 4 then
            B738_autostart_step_completed(autostart.step, autostart.phase[autostart.step], "LEFT APU GENERATOR OFF")
        end

    ----- AUTO-START STEP 27: APU GENERATOR R OFF
    elseif autostart.step == 27 then

        -- PHASE 1: SET THE SWITCH
        if autostart.phase[autostart.step] == 1 then
            B738_print_sequence_status(autostart.step, autostart.phase[autostart.step], "RIGHT APU GENERATOR TO OFF...")  -- PRINT THE START PHASE MESSAGE
           	B738CMD_apuR_switch_up:once()													-- APU R GEN OFF
            autostart.phase[autostart.step] = 2                                             -- INCREMENT THE PHASE
        end
        ...
        -- PHASE 4: COMPLETE THE STEP
        if autostart.phase[autostart.step] == 4 then
            B738_autostart_step_completed(autostart.step, autostart.phase[autostart.step], "RIGHT APU GENERATOR OFF")
        end

    ----- AUTO-START STEP 28: APU OFF
    ...
    ----- AUTO-START STEP 29: ENG 1 TO CONTINUOUS IGNITION
    ...
    ----- AUTO-START STEP 30: ENG 2 TO CONTINUOUS IGNITION
	...
	----- AUTO-BOARD STEP 31: MASTER CAUTION
...
function B738_ai_quick_start()
	
	-- AI
	B738_set_ai_all_modes()	
	B738_set_ai_CD()
	B738_set_ai_ER()
    
    -- ANNUNCIATORS
	B738CMD_ai_annun_quick_start:once()
	
	-- CHRONO
	B738CMD_ai_chrono_quick_start:once()
	
	-- COMMS
	B738CMD_ai_comms_quick_start:once()
	
	-- FIRE
	B738CMD_ai_fire_quick_start:once()
	
	-- GLARESHIELD
	B738CMD_ai_glare_quick_start:once()
	
	-- LIGHTING
	B738CMD_ai_lighting_quick_start:once()

	-- SYSTEMS
	B738CMD_ai_systems_quick_start:once()
	
	-- GEAR
	B738CMD_ai_gear_quick_start:once()
...
function B738_set_ai_all_modes()
